...
public class FrmRegUser extends JDialog {
	...
	private String leerNombre() {
		...
		if (!txtNombre.getText().trim().matches("^(?=.{3,15}$)[A-ZÁÉÍÓÚa-zñáéíóú]+(?:\\s[A-ZÁÉÍÓÚa-zñáéíóú]+)?$")) {
	...
	private String leerApellido() {
		...
		if (!txtApellido.getText().trim().matches("^(?=.{3,20}$)[A-ZÁÉÍÓÚa-zñáéíóú]+(?:\\s[A-ZÁÉÍÓÚa-zñáéíóú]+)?$")) {
